...
func readTrains( data * Data, p int ){
	...
	for i := 1 ; i <= p ; i++ {
		...
		data.trains[ i ].passengers = make ( chan Notification, 10 )
		data.trains[ i ].notifications = make( []Notification, 0 )
		x = z
		list := make( []int, x )
		for j := 0 ; j < x ; j++ {
			fmt.Scanf("%d", &z )
...
func readWorkers( data * Data, w int ){
	var x int
	...
	for i := 1 ; i <= w ; i++ {
		...
		data.workers[ i ].taskChannel = make( chan int, 5 )
		data.workers[ i ].notify = make( chan int, 2 )
		data.workers[ i ].finish = make( chan bool, 2 )
	}
}
...
func createWorkersManager( data * Data ){
	data.workersManager.confirmationChannel = make( chan int, 10 )
	data.workersManager.data = data
}