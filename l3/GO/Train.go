...
func (t * Train) goCurrentLine(data *Data ){
	...
	t.checkIfBroken(data);
	t.notifyAll( data, t.currentLine );
...
func (t * Train ) notifyAll(data * Data, lineID int){

	for _, x := range t.notifications {
		if x.lineID == lineID {
			data.workers[ x.passengerID ].notify <- 1
		}
	}

	it := 0
	for it < len( t.notifications ){
		if t.notifications[ it ].lineID ==lineID{
			t.notifications = append( 
				t.notifications[ :it ],
				t.notifications[ it+1: ]...)
		} else {
			it++
		}
	}
